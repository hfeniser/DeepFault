...
def save_layerwise_relevances(relevant_neurons, filename):
    with h5py.File(filename + '_relevant_neurons.h5', 'w') as hf:
        hf.create_dataset("relevant_neurons", data=relevant_neurons)

    return

def load_layerwise_relevances(filename):
    with h5py.File(filename + '_relevant_neurons.h5', 'r') as hf:
...
def generate_adversarial(original_input, method, model,
                         target=None, target_class=None, sess=None, multi=False, **kwargs):
    ...
    if type(original_input) is list or multi:
